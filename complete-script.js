...
let currentAmount = 150; // Default amount
...
function selectTraining(type) {
    ...
    content.innerHTML = `
        ...
        <div id="trainingRegistration">
            <h2>${title}</h2>
            <form onsubmit="proceedToTrainingPayment(event)">
                <div class="form-row">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" required name="participants" min="1" max="50" onchange="updateTrainingPrice('${type}', ${price})">
                    </div>
                    ` : ''}
                </div>
                ${type === 'ip-support' ? `
                ...
                <div class="payment-info">
                    <h3 id="trainingPrice">$${price} ${type !== 'ip-support' ? 'per participant' : 'consultation fee'}</h3>
                </div>
                <div class="form-buttons">
                    <button type="button" onclick="resetTrainingModal()" class="back-btn">← Back</button>
...
function updateTrainingPrice(type, basePrice) {
    ...
    const priceElement = document.getElementById('trainingPrice');
    if (priceElement) {
        priceElement.textContent = `$${totalPrice} (${participants} × $${basePrice})`;
    }
}

function proceedToTrainingPayment(event) {
    event.preventDefault();
    closeModal('trainingModal');
    showServicePaymentModal('training', currentAmount);
}
...
function selectConsultation(type) {
    ...
    switch(type) {
        case 'market-validation':
            currentAmount = 200;
            showConsultationForm('Market Validation Quote Request', true);
            break;
        case 'business-model':
            currentAmount = 300;
            showConsultationForm('Business Model Development', false);
            break;
        case 'mentorship':
            showMentorshipSelection();
            break;
        case 'business-plan':
            currentAmount = 250;
            showConsultationForm('Business Plan Writing Service', false);
            break;
        case 'feasibility':
            showFeasibilityForm();
            break;
        case 'product-development':
            currentAmount = 400;
            showConsultationForm('Product Development & Sensory Evaluation', false);
            break;
    }
}

function showConsultationForm(title, isQuote) {
    const modal = document.getElementById('consultationModal');
    const content = modal.querySelector('.modal-content');
    
    const submitText = isQuote ? 'Request Quote' : 'Continue to Payment';
    
    content.innerHTML = `
        <span class="close" onclick="closeModal('consultationModal')">&times;</span>
        <div id="consultationForm">
            <h2>${title}</h2>
            <form onsubmit="proceedToConsultationPayment(event, ${isQuote})">
                <div class="form-row">
                    <div class="form-group">
                        <label>Full Name *</label>
                        <input type="text" required name="fullName">
                ...
                <div class="payment-info">
                    <h3>$${currentAmount} ${isQuote ? '(Estimated)' : ''}</h3>
                </div>
                <div class="form-buttons">
                    <button type="button" onclick="resetConsultationModal()" class="back-btn">← Back</button>
...
function showMentorshipSelection() {
    const modal = document.getElementById('consultationModal');
    const content = modal.querySelector('.modal-content');
...
function showFeasibilityForm() {
    ...
    content.innerHTML = `
            ...
            <form onsubmit="return false;">
                ...
                <div class="form-group">
                    ...
                    <div class="file-upload">
                        ...
                        <div class="file-upload-label">
                            Click to upload your business concept documents. (Word/Pdf/Ppt)
                        </div>
                    </div>
                </div>
                <div class="form-buttons">
        ...
        <div id="feasibilityResults" style="display: none;">
            ...
            <div class="form-buttons">
                <button type="button" onclick="showFeasibilityForm()" class="back-btn">← Back</button>
...
function proceedToConsultationPayment(event, isQuote) {
    event.preventDefault();
    
    if (isQuote) {
        closeModal('consultationModal');
        showSuccessMessage('Quote Requested', "We'll contact you via email/phone with a detailed quote.");
    } else {
        closeModal('consultationModal');
        showServicePaymentModal('consultation', currentAmount);
    }
}
...
function selectMentorCard(index) {
    ...
    currentAmount = selectedMentor.price;
...
function proceedToMentorPayment() {
    ...
    closeModal('consultationModal');
    showServicePaymentModal('mentorship', currentAmount);
}
...
function selectFacility(type) {
    ...
    content.innerHTML = `
        ...
        <div id="facilityBooking">
            <h2>${title}</h2>
            <form onsubmit="proceedToFacilityPayment(event)">
                <div class="form-row">
                    <div class="form-group">
                        <label>Full Name *</label>
                        <input type="text" required name="fullName">
                ...
                <div class="form-buttons">
                    <button type="button" onclick="resetFacilityModal()" class="back-btn">← Back</button>
                    <button type="submit" class="submit-btn">Continue to Payment</button>
                </div>
            </form>
        </div>
...
function proceedToFacilityPayment(event) {
    event.preventDefault();
    closeModal('facilityModal');
    showServicePaymentModal('facility', currentAmount);
}
...
function showEventDetails(day) {
    const events = [
        { title: 'Startup Pitch Night', time: '6:00 PM - 9:00 PM', description: 'Present your startup idea to investors and mentors.', price: 50 },
        { title: 'Entrepreneurship Workshop', time: '2:00 PM - 5:00 PM', description: 'Learn the fundamentals of starting a business.', price: 75 },
        { title: 'Networking Mixer', time: '7:00 PM - 10:00 PM', description: 'Connect with fellow entrepreneurs and industry professionals.', price: 30 },
        { title: 'Tech Innovation Seminar', time: '10:00 AM - 12:00 PM', description: 'Explore the latest trends in technology and innovation.', price: 60 },
        { title: 'Business Model Canvas Workshop', time: '1:00 PM - 4:00 PM', description: 'Design and validate your business model.', price: 80 },
        { title: 'Investor Meetup', time: '5:00 PM - 8:00 PM', description: 'Meet potential investors for your startup.', price: 40 }
    ];
    ...
    if (eventInfo) {
        eventInfo.innerHTML = selectedEvents.map(event => `
            <div class="event-item">
                ...
                <button class="event-register-btn" onclick="registerForEvent('${event.title}', ${event.price})">Register</button>
            </div>
        `).join('');
    }
...
function registerForEvent(eventTitle, eventPrice) {
    currentAmount = eventPrice;
    closeModal('networkingModal');
    showServicePaymentModal('networking', currentAmount, eventTitle);
}

// Payment Integration Functions
function showServicePaymentModal(serviceType, amount, eventTitle = '') {
    // Create a temporary payment modal
    ...
    const modalHTML = `
        <div id="servicePaymentModal" class="modal" style="display: block;">
            <div class="modal-content">
                <span class="close" onclick="closeServicePaymentModal()">&times;</span>
                <h2>Payment for ${serviceType.charAt(0).toUpperCase() + serviceType.slice(1)}${eventTitle ? ` - ${eventTitle}` : ''}</h2>
                <div class="payment-info">
                    <h3>Amount: $${amount}</h3>
                </div>
                
                <!-- Payment Method Selection -->
                <div class="payment-methods" id="servicePaymentMethods">
                    <button class="payment-method-btn" onclick="selectServicePaymentMethod('card')">
                        <div class="icon">
                            <i class="fas fa-credit-card"></i>
                        </div>
                        <span>Card Payment</span>
                    </button>
                    <button class="payment-method-btn" onclick="selectServicePaymentMethod('ewallet')">
                        <div class="icon">
                ...
                <div class="payment-form-container" id="serviceCardContainer" style="display: none;">
                    <div class="form-header">
                        <button class="back-btn" onclick="resetServicePaymentModal()">← Back</button>
                        <div class="section-title">Card Details</div>
                    </div>
                    
                    <div class="form-group">
                        <input type="text" id="serviceCardNumber" placeholder="1234 5678 9012 3456" maxlength="19">
                        <div class="validation-message" id="serviceCardNumberValidation"></div>
                    </div>

                    <div class="form-row">
                        <div class="form-col">
                            <input type="text" id="serviceExpiryDate" placeholder="MM/YY" maxlength="5">
                            <div class="validation-message" id="serviceExpiryValidation"></div>
                        </div>
                        <div class="form-col">
                            <input type="text" id="serviceCvv" placeholder="123" maxlength="4">
                            <div class="validation-message" id="serviceCvvValidation"></div>
                        </div>
                    </div>

                    <div class="form-group">
                        <input type="text" id="serviceCardholderName" placeholder="Full name on card">
                        <div class="validation-message" id="serviceNameValidation"></div>
                    </div>

                    <button class="pay-button" onclick="processServiceCardPayment()">
                        <span>Pay $${amount}</span>
                    </button>
                </div>

                <!-- E-Wallet Container -->
                <div class="payment-form-container" id="serviceEwalletContainer" style="display: none;">
                    <div class="form-header">
                        <button class="back-btn" onclick="resetServicePaymentModal()">← Back</button>
                        <div class="section-title">Choose E-Wallet Provider</div>
                    </div>
                    
                    <div class="ewallet-options">
                        <div class="ewallet-btn" onclick="selectServiceEWallet('gcash')">
                        ...
                        <div class="ewallet-btn" onclick="selectServiceEWallet('maya')">
                            <div class="ewallet-left">
                                <div class="ewallet-icon maya">M</div>
                                <span>Maya</span>
                            </div>
                        </div>
                        
                        <div class="ewallet-btn" onclick="selectServiceEWallet('paypal')">
                            <div class="ewallet-left">
                                <div class="ewallet-icon paypal">P</div>
                                <span>PayPal</span>
                            </div>
                        </div>
                    </div>
                ...
                <!-- QR Code Display -->
                <div class="payment-form-container" id="serviceQrContainer" style="display: none;">
                    <div class="form-header">
                        <button class="back-btn" onclick="backToServiceEwalletSelection()">← Back</button>
                        <div class="section-title" id="serviceQrTitle">Pay with E-Wallet</div>
                    </div>
                    
                    <div class="qr-section-large">
                        <div class="qr-container-large" id="serviceQrDisplay">
                            <img src="gcash-qr.png" alt="QR Code" class="qr-image-large">
                        </div>
                        <div class="qr-amount">Amount: $${amount}</div>
                        <div class="qr-instructions" id="serviceQrInstructions">Scan the QR code to complete payment</div>
                        
                        <button class="pay-button" onclick="completeServiceEWalletPayment()">
    ...
    currentModal = document.getElementById('servicePaymentModal');
    
    // Initialize service payment validation
    initializeServiceCardValidation();
}

function selectServicePaymentMethod(method) {
    const methodsContainer = document.getElementById('servicePaymentMethods');
    const cardContainer = document.getElementById('serviceCardContainer');
    const ewalletContainer = document.getElementById('serviceEwalletContainer');
...
function selectServiceEWallet(wallet) {
    ...
    const ewalletContainer = document.getElementById('serviceEwalletContainer');
    const qrContainer = document.getElementById('serviceQrContainer');
    const qrTitle = document.getElementById('serviceQrTitle');
    const qrDisplay = document.getElementById('serviceQrDisplay');
    const qrInstructions = document.getElementById('serviceQrInstructions');
    ...
    qrTitle.textContent = `Pay with ${walletNames[wallet]}`;
    qrInstructions.textContent = `Scan the QR code with your ${walletNames[wallet]} app to complete payment`;
    
    if (wallet === 'gcash') {
        qrDisplay.innerHTML = '<img src="gcash-qr.png" alt="GCash QR Code" class="qr-image-large">';
    } else if (wallet === 'maya') {
        qrDisplay.innerHTML = '<img src="maya-qr.png" alt="Maya QR Code" class="qr-image-large">';
    } else if (wallet === 'paypal') {
        qrDisplay.innerHTML = `
            <div class="qr-error-large">
                <i class="fas fa-exclamation-triangle"></i>
                <div>QR Code unavailable</div>
                <div>Please try another payment method</div>
            </div>
        `;
    }
}

function resetServicePaymentModal() {
    const methodsContainer = document.getElementById('servicePaymentMethods');
    const cardContainer = document.getElementById('serviceCardContainer');
    const ewalletContainer = document.getElementById('serviceEwalletContainer');
    const qrContainer = document.getElementById('serviceQrContainer');
...
function backToServiceEwalletSelection() {
    const ewalletContainer = document.getElementById('serviceEwalletContainer');
    const qrContainer = document.getElementById('serviceQrContainer');
...
function initializeServiceCardValidation() {
    setTimeout(() => {
        const cardNumber = document.getElementById('serviceCardNumber');
        const expiryDate = document.getElementById('serviceExpiryDate');
        const cvv = document.getElementById('serviceCvv');
        const cardholderName = document.getElementById('serviceCardholderName');
        ...
        cardNumber.addEventListener('input', function(e) {
            ...
            validateServiceCardNumber(value);
        });
        ...
        expiryDate.addEventListener('input', function(e) {
            ...
            validateServiceExpiryDate(value);
        });
        
        cvv.addEventListener('input', function(e) {
            let value = e.target.value.replace(/\D/g, '');
            e.target.value = value;
            validateServiceCVV(value);
        });
        
        cardholderName.addEventListener('input', function(e) {
            validateServiceCardholderName(e.target.value);
        });
    }, 100);
}

function validateServiceCardNumber(number) {
    const input = document.getElementById('serviceCardNumber');
    const validation = document.getElementById('serviceCardNumberValidation');
...
function validateServiceExpiryDate(date) {
    const input = document.getElementById('serviceExpiryDate');
    const validation = document.getElementById('serviceExpiryValidation');
...
function validateServiceCVV(cvv) {
    const input = document.getElementById('serviceCvv');
    const validation = document.getElementById('serviceCvvValidation');
...
function validateServiceCardholderName(name) {
    const input = document.getElementById('serviceCardholderName');
    const validation = document.getElementById('serviceNameValidation');
...
function validateServiceCardForm() {
    const cardNumber = document.getElementById('serviceCardNumber');
    const expiryDate = document.getElementById('serviceExpiryDate');
    const cvv = document.getElementById('serviceCvv');
    const cardholderName = document.getElementById('serviceCardholderName');
    ...
    const isCardValid = validateServiceCardNumber(cardNumberValue);
    const isExpiryValid = validateServiceExpiryDate(expiryValue);
    const isCVVValid = validateServiceCVV(cvvValue);
    const isNameValid = validateServiceCardholderName(nameValue);
    
    return isCardValid && isExpiryValid && isCVVValid && isNameValid;
}

function processServiceCardPayment() {
    if (isProcessing) return;
    
    if (!validateServiceCardForm()) {
    ...
    setTimeout(() => {
        ...
        showSuccessMessage('Payment Successful!', `Your payment of $${currentAmount} has been processed successfully.`);
    }, 3000);
}
...
function completeServiceEWalletPayment() {
    ...
    setTimeout(() => {
        ...
        closeServicePaymentModal();
        showSuccessMessage('Payment Successful!', `Your payment of $${currentAmount} has been processed successfully via ${selectedEWallet}.`);
    }, 2000);
}
...
// Payment method selection for retainer modal
...
// E-wallet selection for retainer modal
...
// Navigation functions for retainer modal
...
// Card validation initialization for retainer modal
...
// Process card payment for retainer modal
...
// Complete e-wallet payment for retainer modal
...
// Complete payment (success simulation) for retainer modal
...
// Reset payment form for retainer modal
...
function goBackToTrainingRegistration() {
    // Not needed as we proceed directly to payment modal
}
...
// Enhanced payment processing for specific modals
...
document.addEventListener('keypress', function(e) {
    if (e.key === 'Enter' && e.target.tagName === 'INPUT') {
        ...
        if (currentPaymentMethod === 'card') {
            if (document.getElementById('serviceCardNumber')) {
                processServiceCardPayment();
            } else {
                processCardPayment();
            }
        } else if (currentPaymentMethod === 'ewallet' && selectedEWallet) {
            if (document.getElementById('serviceQrContainer')) {
                completeServiceEWalletPayment();
            } else {
...
window.resetTrainingModal = resetTrainingModal;
window.resetConsultationModal = resetConsultationModal;
window.resetFacilityModal = resetFacilityModal;
window.proceedToTrainingPayment = proceedToTrainingPayment;
window.proceedToConsultationPayment = proceedToConsultationPayment;
window.proceedToFacilityPayment = proceedToFacilityPayment;
window.showServicePaymentModal = showServicePaymentModal;
window.selectServicePaymentMethod = selectServicePaymentMethod;
window.selectServiceEWallet = selectServiceEWallet;
window.resetServicePaymentModal = resetServicePaymentModal;
window.backToServiceEwalletSelection = backToServiceEwalletSelection;
window.closeServicePaymentModal = closeServicePaymentModal;
window.processServiceCardPayment = processServiceCardPayment;
window.completeServiceEWalletPayment = completeServiceEWalletPayment;
